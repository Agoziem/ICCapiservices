...
from ..schemas import (
    CommentSchema, CreateCommentSchema, UpdateCommentSchema,
    ErrorResponseSchema, SuccessResponseSchema, CommentListResponseSchema
)
...
class CommentsController:
    ...
    @http_get('/blog/{blog_id}', response={200: List[CommentSchema], 404: str, 500: str})
    def get_blog_comments(self, blog_id: int):
        """Get all comments for a specific blog"""
        try:
            blog = get_object_or_404(Blog, id=blog_id)
            comments = Comment.objects.filter(blog=blog).select_related('user').order_by('-date')
    ...
    @http_post('/blog/{blog_id}/user/{user_id}', response={201: CommentSchema, 400: ErrorResponseSchema, 404: str, 500: str})
    ...
    @http_get('/user/{user_id}', response={200: List[CommentSchema], 404: str, 500: str})
    def get_user_comments(self, user_id: int):
        """Get all comments by a specific user"""
        try:
            user = get_object_or_404(User, id=user_id)
            comments = Comment.objects.filter(user=user).select_related('blog').order_by('-date')
            return 200, [CommentSchema.model_validate(comment) for comment in comments]
        except Http404:
