...
from ..schemas import (
    BlogSchema, CreateBlogSchema, UpdateBlogSchema, BlogSummarySchema,
    BlogWithCommentsSchema, ErrorResponseSchema, SuccessResponseSchema,
    BlogListResponseSchema, BlogStatsSchema
)
...
class BlogsController:
    ...
    @http_get('/organization/{organization_id}', response={200: List[BlogSchema], 404: str, 500: str})
    def get_org_blogs(self, organization_id: int):
        """Get all blogs for a specific organization"""
        try:
            organization = get_object_or_404(Organization, id=organization_id)
            blogs = Blog.objects.filter(organization=organization).select_related('author', 'category').prefetch_related('tags', 'likes')
    ...
    def get_user_blogs(self, user_id: int):
        ...
        try:
            ...
            blogs = Blog.objects.filter(author=user).select_related('author', 'category').prefetch_related('tags', 'likes')
            return 200, [BlogSchema.model_validate(blog) for blog in blogs]
        except Http404:
    ...
    def get_blog(self, blog_id: int):
        ...
        try:
            blog = get_object_or_404(
                ...
                .prefetch_related('tags', 'likes', 'comment_set__user'),
                id=blog_id
            )
    ...
    @http_get('/slug/{slug}', response={200: BlogWithCommentsSchema, 404: str, 500: str})
    ...
    @http_post('/organization/{organization_id}/user/{user_id}', response={201: BlogSchema, 400: ErrorResponseSchema, 404: str, 500: str})
    def create_blog(self, organization_id: int, user_id: int, data: CreateBlogSchema, img: Optional[File[UploadedFile]] = None):
    ...
    @http_put('/{blog_id}', response={200: BlogSchema, 400: ErrorResponseSchema, 404: str, 500: str})
    def update_blog(self, blog_id: int, data: UpdateBlogSchema, img: Optional[File[UploadedFile]] = None):
    ...
    @http_post('/{blog_id}/view', response={200: SuccessResponseSchema, 404: str, 500: str})
    ...
    def get_all_blogs(self):
        ...
        try:
            blogs = Blog.objects.select_related('author', 'category').prefetch_related('likes')
