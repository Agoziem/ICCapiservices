...
from ..schemas import (
    QuestionSchema, StudentTestSchema, StudentTestDetailSchema, StudentTestRequestSchema,
...
class PracticeController:
    ...
    def get_available_tests(self, request, year_id: Optional[int] = None, test_type_id: Optional[int] = None):
        """Get all available tests for practice"""
        tests = Test.objects.select_related('testYear', 'texttype').prefetch_related('testSubject')
        ...
        for test in tests:
            ...
            for subject in test.testSubject.all():
                ...
                marks = subject.questions.aggregate(total=Sum('questionMark'))['total'] or 0
    ...
    def start_test(self, request, payload: StudentTestRequestSchema):
    ...
    def submit_test(self, request, payload: SubmitStudentTestSchema):
        ...
        for submission in payload.submissions:
            ...
            for question_answer in submission.questions:
                ...
                subject = question.subject_set.first()
                ...
                if question_answer.selected_answer_id:
                    selected_answer = get_object_or_404(Answer, id=question_answer.selected_answer_id)
    ...
    def get_my_test_results(self, request):
        ...
        test_results = TestResult.objects.filter(user=request.user).prefetch_related('tests')
    ...
    def get_test_result(self, request, result_id: int):
        ...
        test_result = get_object_or_404(
            TestResult.objects.prefetch_related('tests__testSubject__questions__answers'),
            id=result_id,
            user=request.user
        )
        return test_result
...
class ComputingController:
    ...
    @route.get('/tests')
    def get_computing_tests(self, request):
        """Get all computing-related tests"""
        computing_tests = Test.objects.filter(
            Q(texttype__testtype__icontains='computing') |
            Q(testSubject__subjectname__icontains='computer')
        ).distinct().select_related('testYear', 'texttype').prefetch_related('testSubject')
        
        return [StudentTestSchema.model_validate(test) for test in computing_tests]
    
    @route.get('/subjects')
    def get_computing_subjects(self, request):
        """Get all computing-related subjects"""
        computing_subjects = Subject.objects.filter(
            subjectname__icontains='computer'
    ...
    @route.get('/practice-questions')
    def get_computing_practice_questions(self, request, limit: Optional[int] = 10):
        """Get random computing questions for practice"""
        questions = Question.objects.filter(
            subject__subjectname__icontains='computer'
        ).prefetch_related('answers').order_by('?')[:limit]
