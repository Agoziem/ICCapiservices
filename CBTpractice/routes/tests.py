...
from ..schemas import (
    YearSchema, YearListResponseSchema, CreateYearSchema, UpdateYearSchema,
    TestTypeSchema, TestTypeListResponseSchema, CreateTestTypeSchema, UpdateTestTypeSchema,
    TestSchema, TestListResponseSchema, TestSummarySchema, CreateTestSchema, UpdateTestSchema,
...
class YearsController:
    ...
    @route.delete('/{year_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
...
class TestTypesController:
    ...
    @route.put('/{test_type_id}', response=TestTypeSchema, permissions=[IsAuthenticated])
    ...
    @route.delete('/{test_type_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
...
class TestsController:
    ...
    def list_tests(self, year_id: Optional[int] = None, test_type_id: Optional[int] = None):
    ...
    def get_test(self, test_id: int):
        ...
        test = get_object_or_404(
            Test.objects.select_related('testYear', 'texttype')
                       .prefetch_related('testSubject__questions__answers'),
            id=test_id
        )
        return test
    ...
    @route.delete('/{test_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
    ...
    def get_test_summary(self, test_id: int):
        ...
        test = get_object_or_404(
            Test.objects.select_related('testYear', 'texttype')
                       .annotate(subjects_count=Count('testSubject')),
            id=test_id
        )
        return test
