...
from ..schemas import (
    DepartmentSchema, DepartmentListResponseSchema, PaginatedDepartmentResponseSchema,
    DepartmentServiceSchema, DepartmentServiceListResponseSchema,
    CreateDepartmentSchema, UpdateDepartmentSchema,
    CreateDepartmentServiceSchema, UpdateDepartmentServiceSchema,
...
class DepartmentsController:
    ...
    def list_departments(self, organization_id: int, page: Optional[int] = 1, page_size: Optional[int] = 10):
        """Get all departments for an organization with pagination"""
        try:
            departments = Department.objects.filter(organization=organization_id).order_by('name')
            if not page_size:
                page_size = 10
    ...
    @route.post('/{organization_id}', response=DepartmentSchema, permissions=[IsAuthenticated])
    ...
    @route.put('/{department_id}', response=DepartmentSchema, permissions=[IsAuthenticated])
    ...
    @route.delete('/{department_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
...
class DepartmentServicesController:
    ...
    @route.put('/{service_id}', response=DepartmentServiceSchema, permissions=[IsAuthenticated])
    ...
    @route.delete('/{service_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
