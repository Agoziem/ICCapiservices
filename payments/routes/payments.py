...
from ..schemas import (
    OrderSchema, OrderListResponseSchema,
    CreateOrderSchema, UpdateOrderSchema, VerifyPaymentSchema,
    PaymentStatsSchema, PaymentVerificationResponseSchema,
...
class PaymentsController:
    ...
    def get_payments(self, organization_id: int):
        ...
        orders = Orders.objects.filter(organization=organization_id).select_related(
    ...
    def get_payments_by_user(self, user_id: int):
        ...
        orders = Orders.objects.filter(customer=user_id).select_related(
            'organization', 'customer'
        ).prefetch_related('services', 'products', 'videos')
        return orders
    ...
    def get_payment(self, payment_id: int):
        ...
        order = get_object_or_404(
            Orders.objects.select_related('organization', 'customer')
                          .prefetch_related('services', 'products', 'videos'),
            id=payment_id
        )
        return order
    
    @route.post('/{organization_id}', response=OrderSchema, permissions=[IsAuthenticated])
    ...
    @route.delete('/{payment_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
    ...
    @route.post('/{payment_id}/mark-delivered', response=OrderSchema, permissions=[IsAuthenticated])
    ...
    def get_pending_payments(self, organization_id: int):
        ...
        orders = Orders.objects.filter(
            ...
            status='Pending'
        ).select_related('organization', 'customer').prefetch_related('services', 'products', 'videos')
        return orders
