...
from ..schemas import (
    ServiceSchema, ServiceListResponseSchema,
    CreateServiceSchema, UpdateServiceSchema, ServiceFileUploadSchema,
    SuccessResponseSchema, ErrorResponseSchema,
    ServiceUserDetailsSchema, ServiceUserListResponseSchema
)
...
class ServicesController:
    ...
    def get_services(self, organization_id: int, category: Optional[str] = None):
        ...
        queryset = Service.objects.filter(organization=organization_id).select_related(
    ...
    @paginate(ServicePagination)
    def get_trending_services(self, organization_id: int, category: Optional[str] = None):
    ...
    @paginate(ServicePagination)
    def get_user_services(self, organization_id: int, user_id: int, category: Optional[str] = None):
    ...
    @route.post('/{organization_id}', response=ServiceSchema, permissions=[IsAuthenticated])
    def create_service(self, organization_id: int, payload: CreateServiceSchema, file_data: Optional[ServiceFileUploadSchema] = None):
        ...
        try:
            ...
            if service_data.get('subcategory'):
                subcategory = get_object_or_404(SubCategory, id=service_data.pop('subcategory'))
    ...
    @route.put('/service/{service_id}', response=ServiceSchema, permissions=[IsAuthenticated])
    def update_service(self, service_id: int, payload: UpdateServiceSchema, file_data: Optional[ServiceFileUploadSchema] = None):
        ...
        try:
            ...
            if 'organization' in service_data:
                organization = get_object_or_404(Organization, id=service_data.pop('organization'))
    ...
    @route.delete('/service/{service_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
...
class ServiceUserController:
    ...
    def get_users_that_bought_service(self, service_id: int):
        ...
        for user, count in Counter(users).items():
            ...
            user_data.append(ServiceUserDetailsSchema(
                id=user.id,
