...
from ..schemas import (
    CategorySchema, CategoryListResponseSchema,
    SubCategorySchema, SubCategoryListResponseSchema,
...
@api_controller('/service-categories', tags=['Service Categories'])
class ServiceCategoriesController:
    ...
    @route.delete('/{category_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
...
@api_controller('/service-subcategories', tags=['Service Subcategories'])
class ServiceSubCategoriesController:
    
    @route.get('/{category_id}', response=list[SubCategorySchema])
    def get_subcategories(self, category_id: int):
        """Get all subcategories for a specific category"""
        category = get_object_or_404(Category, id=category_id)
        subcategories = SubCategory.objects.filter(category=category).select_related('category').order_by('subcategory')
    ...
    @route.put('/{subcategory_id}', response=SubCategorySchema, permissions=[IsAuthenticated])
    ...
    @route.delete('/{subcategory_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
    ...
    def get_all_subcategories(self):
        ...
        subcategories = SubCategory.objects.select_related('category').order_by('category__category', 'subcategory')
        return subcategories
