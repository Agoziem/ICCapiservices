...
from ..schemas import (
    ProductSchema, ProductListResponseSchema, PaginatedProductResponseSchema,
    CreateProductSchema, UpdateProductSchema, ProductFileUploadSchema,
    TrendingProductsResponseSchema, UserProductsResponseSchema,
...
class ProductsController:
    ...
    def get_products(self, organization_id: int, category: Optional[str] = None, page: Optional[int] = 1, page_size: Optional[int] = 10):
        ...
        try:
            ...
            products = products.select_related('organization', 'category', 'subcategory__category')
            if not page_size:
                page_size = 10
    ...
    def get_trending_products(self, organization_id: int, category: Optional[str] = None, page: Optional[int] = 1, page_size: Optional[int] = 10):
    ...
    @route.get('/user/{organization_id}/{user_id}', response=PaginatedProductResponseSchema)
    def get_user_products(self, organization_id: int, user_id: int, category: Optional[str] = None, page: Optional[int] = 1, page_size: Optional[int] = 10):
    ...
    def get_product(self, product_id: int):
        ...
        product = get_object_or_404(
            Product.objects.select_related('organization', 'category', 'subcategory__category'),
            id=product_id
        )
        return product
    
    @route.post('/{organization_id}', response=ProductSchema, permissions=[IsAuthenticated])
    def add_product(
        self, 
        organization_id: int, 
    ...
    @route.put('/product/{product_id}', response=ProductSchema, permissions=[IsAuthenticated])
    def update_product(
        self, 
        product_id: int, 
    ...
    @route.delete('/product/{product_id}', response=SuccessResponseSchema, permissions=[IsAuthenticated])
    ...
    def get_free_products(self, organization_id: int):
        ...
        products = Product.objects.filter(
            organization=organization_id,
            free=True
        ).select_related('organization', 'category', 'subcategory__category').order_by('-last_updated_date')
        return products
    ...
    def search_products(self, organization_id: int, query: str):
        ...
        ).select_related('organization', 'category', 'subcategory__category').order_by('-last_updated_date')[:20]
        return products
