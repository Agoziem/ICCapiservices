...
class VideosController:
    ...
    def get_videos(self, organization_id: int, category: Optional[str] = None):
        ...
        queryset = (
            Video.objects.filter(organization=organization_id)
    ...
    def get_trending_videos(self, organization_id: int, category: Optional[str] = None):
        ...
        queryset = (
            Video.objects.filter(organization=organization_id)
            .select_related("organization", "category", "subcategory")
            .annotate(watchers_count=Count("userIDs_that_bought_this_video"))
            .filter(watchers_count__gt=0)
    ...
    def get_free_videos(self, organization_id: int, category: Optional[str] = None):
        ...
        queryset = (
            Video.objects.filter(organization=organization_id, free=True)
            .select_related("organization", "category", "subcategory")
            .order_by("-updated_at")
        )
    ...
    def get_paid_videos(self, organization_id: int, category: Optional[str] = None):
        ...
        queryset = (
            Video.objects.filter(organization=organization_id, free=False)
            .select_related("organization", "category", "subcategory")
            .order_by("-updated_at")
        )
