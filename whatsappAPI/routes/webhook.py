...
from ..schemas import (
    WebhookPayloadSchema, SendMessageSchema, TemplateMessageSchema,
    SuccessResponseSchema, ErrorResponseSchema,
    CreateWebhookEventSchema, WebhookEventSchema
)


@api_controller('/whatsapp/webhook', tags=['WhatsApp Webhook Integration'], permissions=[AllowAny])
class WhatsAppWebhookController:
    ...
    def send_template_message(self, payload: TemplateMessageSchema):
        ...
        try:
            ...
            if response and response.get("messages"):
                return {"status": "success", "message_id": response["messages"][0]["id"]}
            else:
    ...
    def _process_incoming_message(self, message_data):
        ...
        try:
            with transaction.atomic():
                ...
                if not created and profile_name and contact.profile_name != profile_name:
